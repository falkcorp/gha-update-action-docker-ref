...
This repository is a **GHA composite action** that updates `action.yml` with Docker image
references (digest + version). Implementation: `src/update_docker_ref.py` (Python) called
from the composite `runs:` block in `action.yml`.
